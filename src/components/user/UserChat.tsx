...
function UserChat({ trainer }: TrainerChatProps) {
    ...
    const fetchTrainerData = async () => {
      try {
        const response = await userAxiosInstance(
          `${API_URL}/api/user/trainers/${trainer.trainerId}`
        );
  ...
  return (
    <div className="w-full h-[85vh] flex flex-col bg-gray-900 text-white shadow-xl rounded-lg border border-gray-700 overflow-hidden pt-6">
      {/* HEADER */}
      <div className="bg-gradient-to-r from-[#572c52] to-[#572c52] px-5 py-3 flex items-center sticky top-20 z-20 shadow-md">
        <img
      ...
      <div className="flex-1 overflow-y-auto px-4 mt-2 overflow-x-hidden">
      ...
      {/* FOOTER */}
      <div className="px-4 py-2 border-t border-gray-700 bg-gray-800 sticky bottom-0 z-20">
        <MessageInpurBar
